...
def test_pil_heic_support():
    ...
    assert heic_supported, f"HEIC format not supported. Available formats: {list(supported_formats)}"
    ...
    assert len(found_extensions) > 0, f"No HEIC extensions found in supported formats: {list(supported_formats)}"
